...
var objScor = document.createElement("div");
var elScor = document.createElement("div");
objScor.append(elScor);
...
function haveFood(unit) {
    ...
    if (unit_classes.includes('food-unit')) {
        check = true;
        createFood();
        setTimeout(createBarrier, 2000);
        score++;

        elScor.innerText = "Ваш счет -" + score;
        document.querySelector(".buttons").append(objScor);
    }
    return check;
}

function haveBarrier(un) {
    var checked = false;

    var unitClasses = un.getAttribute('class').split(' ');
